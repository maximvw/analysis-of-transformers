...
def train_epoch(
    model: GraphGPT | torch.nn.DataParallel,
    probe: DSUProbe | torch.nn.DataParallel | None,
    loader: DataLoader,
...
def _atomic_torch_save(state: dict, checkpoint_path: Path):
...
def save_checkpoint(
    ...
    state = _build_checkpoint_state(
        model, probe, optimizer, epoch, metrics, include_optimizer=include_optimizer
    )
    _atomic_torch_save(state, checkpoint_path)
    info_path = checkpoint_path.parent / "best_checkpoint_info.json"
...
def train(args: argparse.Namespace):
    ...
    train_ds = GraphConnectivityDataset(
        args.train_path, tokenizer, max_n=args.max_n, fixed=False, dsu_mode=args.dsu_mode
    )
    val_ds = GraphConnectivityDataset(
        args.val_path, tokenizer, max_n=args.max_n, fixed=True, dsu_mode=args.dsu_mode
    )
    ...
    for epoch in range(args.epochs):
        ...
        # Save only the single best checkpoint when validation improves.
...
def main():
    ...
    parser.add_argument("--dsu_mode", type=str, default="min", choices=["min", "size", "rank"],
                        help="DSU union strategy (min, size, rank).")
    # Misc
