"""Training loop for topological graph sorting models."""
...
from .tokenizer import TopologicalSortTokenizer


def train_epoch(
    model: TopologicalSortGPT | torch.nn.DataParallel,
    probe: InDegreeProbe | torch.nn.DataParallel | None,
    loader: DataLoader,
        ...
        json.dump(
            {
                "best_epoch": epoch,
                "metrics": metrics,
                "checkpoint_path": str(save_dir / "checkpoint_best.pt"),
            },
            f,
            indent=2,
        )
...
def train(args: argparse.Namespace):
    ...
    tokenizer = TopologicalSortTokenizer(max_n=args.max_n)
    print(f"Vocab size: {tokenizer.vocab_size}")

    train_ds = TopologicalSortDataset(args.train_path, tokenizer)
    val_ds = TopologicalSortDataset(args.val_path, tokenizer)
...
def main():
    parser = argparse.ArgumentParser(description="Train topological graph sorting model")
    parser.add_argument("--train_path", type=str, required=True)
    parser.add_argument("--val_path", type=str, required=True)
    parser.add_argument("--max_n", type=int, default=18)
    parser.add_argument("--max_indegree", type=int, default=18)
    ...
    parser.add_argument("--save_dir", type=str, default="checkpoints/topological_sorting")
