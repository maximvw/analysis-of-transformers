"""Training loop for string edit sequence models."""
...
from .tokenizer import StringEditTokenizer


def train_epoch(
    model: StringEditGPT | torch.nn.DataParallel,
    probe: CursorStateProbe | torch.nn.DataParallel | None,
    loader: DataLoader,
...
def train(args: argparse.Namespace):
    ...
    train_ds = StringEditDataset(args.train_path, tokenizer)
    val_ds = StringEditDataset(args.val_path, tokenizer)
    ...
    if args.lambda_state > 0:
        probe = CursorStateProbe(
            ...
            max_cursor_pos=args.max_cursor_pos,
        ).to(device)
...
def main():
    parser = argparse.ArgumentParser(description="Train string edit sequence model")
    ...
    parser.add_argument("--max_cursor_pos", type=int, default=33)
    ...
    parser.add_argument("--save_dir", type=str, default="checkpoints/string_edit_sequence")
